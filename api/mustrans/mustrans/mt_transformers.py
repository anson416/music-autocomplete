...
class MTTransformers(object):
    def __init__(
        self,
        model: Literal["ismir2021", "mt3",] = "ismir2021",
        model_dir: str = "./models/mustrans_transformers",
        mt3_dir: str = "./mustrans/mt3/mt3"
    ) -> None:
        if model == "ismir2021":
        ...
        if not model_dir.exists():
            zip_path = model_dir / "checkpoints.zip"
            model_dir.mkdir(parents=True, exist_ok=True)
            gdown.download(
                url="https://drive.google.com/file/d/1QIMHkHkvKbBARu4AqX0ayGGwZZHw4OeH/view?usp=share_link",
                output=str(zip_path),
                quiet=False,
                fuzzy=True,
            )
            with ZipFile(zip_path, "r") as zObject:
                zObject.extractall(model_dir)
            zip_path.unlink()

        checkpoint_dir = os.path.join(model_dir, "checkpoints", model)
        gin_files = [
            os.path.join(mt3_dir, "gin", "model.gin"),
            os.path.join(mt3_dir, "gin", f"{model}.gin"),
        ]
    ...
    def __call__(self, audio_path: str, sample_rate: int = 16000):
        with open(audio_path, "rb") as f:
            wav_data = f.read()
        audio = note_seq.audio_io.wav_data_to_samples_librosa(wav_data, sample_rate=sample_rate)
